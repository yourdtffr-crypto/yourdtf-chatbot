...
const SUGGESTIONS = [
  "DTF ou UV DTF, quelle différence ?",
  "Prix pour 20 UV DTF A3 ?",
  "Je veux créer une planche DTF",
  "Comment presser un transfert ?",
]

const TypingDots = () => (
  <div style={{ display: 'flex', gap: 5, alignItems: 'center', padding: '14px 18px', background: 'var(--surface2)', borderRadius: '18px 18px 18px 4px', width: 'fit-content' }}>
    {[0,1,2].map(i => (
      <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--orange)', animation: 'bounce 1.2s infinite', animationDelay: `${i*0.2}s` }} />
    ))}
  </div>
)
...
const TAG_COLORS = {
  'DTF Textile': '#2563eb',
  'UV DTF Rigide': '#7c3aed',
  'Outil en ligne': '#059669',
}

const ProductCard = ({ data }) => (
  <a href={data.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--surface)', border: '1px solid var(--orange)', borderRadius: 12, padding: '12px 16px', textDecoration: 'none', color: 'var(--text)', cursor: 'pointer', marginTop: 8, transition: 'all 0.2s' }}
  ...
  >
    ...
    <div style={{ flex: 1 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        {data.tag && (
          <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 6px', borderRadius: 4, background: TAG_COLORS[data.tag] || '#666', color: 'white', textTransform: 'uppercase', letterSpacing: '0.3px' }}>{data.tag}</span>
        )}
      </div>
...
const PriceCard = ({ data }) => (
  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', marginTop: 8 }}>
    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Estimation de prix</div>
    {[
      ['Produit', data.productName],
      ['Quantité', data.quantity],
      ['Prix unitaire HT', `${data.unitPrice}€`],
    ].map(([label, val]) => (
    ...
    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}>Prix hors taxes — livraison gratuite dès 50€</div>
  </div>
)

const EscalationCard = ({ data }) => {
  const mailto = `mailto:yourdtf.fr@gmail.com?subject=${encodeURIComponent('Client nécessite assistance')}&body=${encodeURIComponent(`Résumé:\n${data.summary}\n\nEmail client: ${data.client_email}`)}`
  useEffect(() => { window.open(mailto, '_blank') }, [])
  return (
    <div style={{ background: 'rgba(255,61,0,0.08)', border: '1px solid var(--orange)', borderRadius: 12, padding: '14px 16px', marginTop: 8 }}>
      <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>📧 Mise en relation avec l'équipe</div>
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>Un email a été préparé pour l'équipe YourDTF. Ils vous répondront rapidement.</div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <a href={mailto} style={{ display: 'inline-block', padding: '8px 14px', background: 'var(--orange)', borderRadius: 8, color: 'white', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>Envoyer l'email →</a>
        <a href="tel:+33328632212" style={{ display: 'inline-block', padding: '8px 14px', background: 'var(--surface2)', borderRadius: 8, color: 'var(--text)', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>📞 03 28 63 22 12</a>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: isUser ? 'flex-end' : 'flex-start', animation: 'fadeIn 0.3s ease forwards' }}>
      <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', width: '100%' }}>
        {!isUser && (
          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--orange)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginRight: 10, marginTop: 2, fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 13, color: 'white' }}>Y</div>
        )}
        {msg.content && (
          <div style={{ maxWidth: '75%', padding: '12px 16px', borderRadius: isUser ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: isUser ? 'linear-gradient(135deg, var(--orange) 0%, var(--orange-dark) 100%)' : 'var(--surface2)', color: 'var(--text)', fontSize: 14, lineHeight: 1.6, fontFamily: 'var(--font-body)', fontWeight: 400, boxShadow: isUser ? '0 4px 20px rgba(255,61,0,0.25)' : 'none', whiteSpace: 'pre-wrap' }}>{msg.content}</div>
        )}
      </div>
...
export default function App() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.message, toolResults: data.toolResults || [] }])
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: "Désolé, une erreur s'est produite. Veuillez réessayer.", toolResults: [] }])
    } finally {
      setLoading(false)
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(17,17,17,0.95)', backdropFilter: 'blur(10px)', flexShrink: 0, zIndex: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Assistant en ligne</span>
          </div>
        </div>
        <a href="tel:+33328632212" style={{ fontSize: 11, color: 'var(--text-muted)', textDecoration: 'none', padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 8 }}>📞 Appeler</a>
      </div>
        ...
        {!started && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 24, animation: 'slideUp 0.5s ease forwards' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 15, color: 'var(--text-muted)', fontWeight: 300 }}>Expert DTF & UV DTF — comment puis-je vous aider ?</div>
            </div>
